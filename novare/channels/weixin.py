"""novare/channels/weixin.py — 个人微信渠道

使用 ilinkai.weixin.qq.com HTTP 长轮询 API 接收和发送个人微信消息。
从 researchbot 项目移植，适配 Novare 架构。
"""
...
logger = logging.getLogger("novare.channels.weixin")
...
# ---------------------------------------------------------------------------
# Workspace paths (replaces researchbot.config.paths)
# ---------------------------------------------------------------------------

_WORKSPACE = Path(os.environ.get("NOVARE_WORKSPACE", "./workspace"))


def _get_media_dir(channel: str = "weixin") -> Path:
    d = _WORKSPACE / ".novare" / "media" / channel
    d.mkdir(parents=True, exist_ok=True)
    return d


def _get_runtime_subdir(name: str) -> Path:
    d = _WORKSPACE / ".novare" / name
    d.mkdir(parents=True, exist_ok=True)
    return d
...
class WeixinConfig(BaseModel):
    ...
    state_dir: str = ""  # Override state directory path
    poll_timeout: int = DEFAULT_LONG_POLL_TIMEOUT_S  # seconds for long-poll
    instance_id: str = ""  # 多实例标识，用于隔离状态目录（由 ChannelManager 注入）
...
class WeixinChannel(BaseChannel):
    ...
    def _get_state_dir(self) -> Path:
        ...
        if self.config.state_dir:
            ...
            # 使用 instance_id 隔离多实例状态
            subdir = f"weixin_{self.config.instance_id}" if self.config.instance_id else "weixin"
            d = _get_runtime_subdir(subdir)
        d.mkdir(parents=True, exist_ok=True)
        self._state_dir = d
        return d
    ...
    async def _poll_once(self) -> None:
        ...
        if is_error:
            if errcode == ERRCODE_SESSION_EXPIRED or ret == ERRCODE_SESSION_EXPIRED:
                ...
                remaining = self._session_pause_remaining_s()
                logger.warning(
                    "WeChat session expired (errcode %s). Pausing %s min.",
                    errcode,
        ...
        try:
            ...
            for idx, (download_source, cdn_url) in enumerate(download_candidates):
                try:
                    ...
                    if should_fallback:
                        logger.warning(
                            "WeChat media download failed via full_url, falling back to encrypt_query_param: type=%s err=%s",
                            media_type,
                            e,
                        )
                        continue
            ...
            media_dir = _get_media_dir(f"weixin_{self.config.instance_id}" if self.config.instance_id else "weixin")
